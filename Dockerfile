...
ENV UV_PROJECT_ENVIRONMENT=/app/.venv \
    ...
    PATH="/app/.venv/bin:/opt/cargo/bin:${PATH}"

# Copy dependency metadata first for layer caching
COPY pyproject.toml uv.lock rust-toolchain.toml ./
COPY litellm-proxy-extras/pyproject.toml litellm-proxy-extras/

# Use the upstream compiler pin; Wolfi's rolling rust package may differ.
# The signed APK supplies rustup, which verifies the toolchain downloads.
...
RUN python3.13 -c 'import subprocess, tomllib; expected = tomllib.load(open("rust-toolchain.toml", "rb"))["toolchain"]["channel"]; actual = subprocess.check_output(["rustc", "--version"], text=True).split()[1]; assert actual == expected, (actual, expected)'

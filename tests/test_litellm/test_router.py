...
async def test_async_function_with_fallbacks_stamps_zero_attempted_fallbacks():
    ...
    caller_snapshot = copy.deepcopy(metadata)
    ...
    assert downstream_calls[0]["metadata"]["attempted_fallbacks"] == 0
    assert downstream_calls[0]["metadata"]["original_model_group"] == "gpt-3.5-turbo"
...
async def test_async_function_with_fallbacks_scrubs_spoofed_values_from_sibling_bucket():
    ...
    assert litellm_metadata["attempted_fallbacks"] == 99
    assert litellm_metadata["original_model_group"] == "spoofed-group"
    assert litellm_metadata["client_key"] == "client_value"
    assert downstream_calls[0]["metadata"]["attempted_fallbacks"] == 0
    assert downstream_calls[0]["metadata"]["original_model_group"] == "gpt-3.5-turbo"
    assert metadata == {}
...
async def test_async_function_with_fallbacks_scrubs_sibling_bucket_in_place():
    """Trusted proxy-owned metadata retains identity and sanitized downstream write-backs."""
    router = litellm.Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
    ...
    request = {"litellm_metadata": litellm_metadata}
    initialize_request_retry_state(request)
    owned_metadata = request["litellm_metadata"]

    await router.acompletion(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hey"}],
        metadata={},
        litellm_metadata=owned_metadata,
    )

    assert len(downstream_calls) == 1
    assert downstream_calls[0]["litellm_metadata"] is owned_metadata
    assert "attempted_fallbacks" not in owned_metadata
    assert "original_model_group" not in owned_metadata
...
async def test_async_function_with_fallbacks_stamps_aliased_buckets_on_every_call():
    """Aliased SDK dictionaries remain reusable while each request owns its attribution."""
    router = litellm.Router(
        model_list=[
            {
    ...
    assert shared_metadata == {"team": "alpha"}
    assert len({id(call["metadata"]) for call in downstream_calls}) == 3
    for call_kwargs in downstream_calls:
        assert call_kwargs["litellm_metadata"] is not shared_metadata
        assert call_kwargs["metadata"] is not shared_metadata
        assert call_kwargs["metadata"]["attempted_fallbacks"] == 0
        assert call_kwargs["metadata"]["original_model_group"] == "chat-group"
        assert "attempted_fallbacks" not in call_kwargs["litellm_metadata"]
        assert "original_model_group" not in call_kwargs["litellm_metadata"]
        assert call_kwargs["litellm_metadata"]["team"] == "alpha"
        assert call_kwargs["metadata"]._retry_state is call_kwargs["litellm_metadata"]._retry_state
...
@pytest.mark.parametrize("limit", [1, 2])
@pytest.mark.parametrize("both_buckets", [False, True])
async def test_request_retry_state_survives_anthropic_deferred_fallback(limit, both_buckets, monkeypatch):
    from litellm.litellm_core_utils.core_helpers import RequestRetryLimitError, get_request_retry_count
    ...
    async def consume():
        response = await router._aanthropic_messages_with_streaming_fallbacks(
            ...
            **({"metadata": {"owner": "synthetic"}, "litellm_metadata": {}} if both_buckets else {}),
        )
...
@pytest.mark.asyncio
@pytest.mark.parametrize("server_side_continuation", [False, True])
async def test_continuation_affinity_keeps_account_boundary_when_upstream_strips_cross_group_reasoning(
    server_side_continuation,
):
    from litellm.responses.utils import ResponsesAPIRequestUtils
    from litellm.router_utils.pre_call_checks.encrypted_content_affinity_check import EncryptedContentAffinityCheck

    deployments = [
        {"model_name": name, "model_info": {"id": name}, "litellm_params": {
            "model": "openai/gpt-4o-mini", "api_base": "https://synthetic.invalid/v1", "api_key": key,
        }}
        for name, key in (("original", "sk-account-a"), ("new-tier", "sk-account-b"))
    ]
    router = Router(model_list=deployments)
    check = EncryptedContentAffinityCheck(router=router)
    wrapped = ResponsesAPIRequestUtils._wrap_encrypted_content_with_model_id("synthetic-ciphertext", "original")
    request = {"input": [
        {"type": "reasoning", "encrypted_content": wrapped},
        {"role": "user", "content": "synthetic readable history"},
    ]}
    if server_side_continuation:
        request["previous_response_id"] = ResponsesAPIRequestUtils._build_responses_api_response_id(
            model_id="original", custom_llm_provider="openai", response_id="resp_synthetic"
        )
    try:
        if server_side_continuation:
            with pytest.raises(litellm.ServiceUnavailableError):
                await check.async_filter_deployments(
                    model="new-tier", healthy_deployments=[deployments[1]], messages=None, request_kwargs=request,
                )

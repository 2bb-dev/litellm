...
class TestPromptCacheParams:
    ...
    def test_existing_positional_cache_arguments_keep_their_bindings(self, func):
        import inspect

        options = {"mode": "implicit", "ttl": "30m"}
        bound = inspect.signature(func).bind(*([None] * 42), "conversation-123", "24h", options)
        assert bound.arguments["prompt_cache_key"] == "conversation-123"
        assert bound.arguments["prompt_cache_retention"] == "24h"
        assert bound.arguments["prompt_cache_options"] == options
        assert "store" not in bound.arguments
    ...
    def test_completion_forwards_params_to_provider_request(self):
        ...
        assert create_kwargs.get("extra_body") == {"prompt_cache_options": {"mode": "implicit", "ttl": "30m"}}
    ...
    def test_responses_bridge_carries_prompt_cache_key(self):
        ...
        assert request.get("extra_body") == {"prompt_cache_options": {"mode": "implicit", "ttl": "30m"}}

...
@pytest.mark.asyncio
@pytest.mark.parametrize("boundary", ["encryption", "terminal"])
async def test_success_collector_keeps_protected_and_terminal_context_in_process(monkeypatch, boundary):
    from litellm.litellm_core_utils.request_content_mode import CONFIG_ENV
    ...
    monkeypatch.delenv("SPEND_LOG_DURABLE_QUEUE_PATH", raising=False)
    ...
    kwargs = _offload_kwargs()
    response = _offload_response()
    session = signed_session()[0]
    if boundary == "encryption":
        monkeypatch.setenv(CONFIG_ENV, "/synthetic-protected.json")
    else:
        kwargs[STAMP] = session
    start_time, end_time = datetime.now(), datetime.now()

    with patch.object(logger, "_PROXY_track_cost_callback", new_callable=AsyncMock) as pipeline:
        await logger.async_log_success_event(kwargs, response, start_time, end_time)

    pipeline.assert_awaited_once_with(kwargs, response, start_time, end_time)
    assert pipeline.await_args.args[0] is kwargs
    assert pipeline.await_args.args[1] is response
    if boundary == "terminal":
        assert pipeline.await_args.args[0][STAMP] is session
        assert pipeline.await_args.args[0][STAMP].envelope is session.envelope
    assert producer.stats().queued == 0
    assert producer.stats().sent == 0
    await producer.close(drain_timeout=1.0)

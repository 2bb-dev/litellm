...
class TestLangfuseUsageDetails(unittest.TestCase):
    ...
    def test_debug_langfuse_dump_carries_no_credentials(self):
        ...
        """
        self._drive_with_canary(extra_metadata={"debug_langfuse": True, "caller_secret": self.CANARY})
    ...
    def test_raw_request_metadata_reaches_the_emitted_blob_through_no_key(self):
        ...
        for caller_key in ("first_custom", "second_custom", "session_id", "trace_name"):
            ...
            assert caller_key not in self.last_trace_kwargs["metadata"]
        assert self.last_trace_kwargs["metadata"] == generation_metadata

    def test_trace_metadata_updates_require_opt_in_and_preserve_explicit_values(self):
        with patch.object(litellm, "langfuse_enable_update_trace_keys", True):
            self._drive_with_canary(extra_metadata={"existing_trace_id": "continued-trace"})
            assert "metadata" not in self.last_trace_kwargs

            self._drive_with_canary(extra_metadata={
                "existing_trace_id": "continued-trace",
                "update_trace_keys": ["trace_metadata"],
                "trace_metadata": {"litellm_response_cost": "explicit-trace-cost", "release": "r1"},
            })

        trace_metadata = self.last_trace_kwargs["metadata"]
        assert trace_metadata["release"] == "r1"
        assert trace_metadata["litellm_response_cost"] == "explicit-trace-cost"
        assert trace_metadata["user_api_key_alias"] == "canary-alias"
        assert self.CANARY not in self._emitted_payload_text()
    ...
    def test_log_langfuse_v2_spend_logs_metadata_promotes_openclaw_tags(self):
        ...
        payload["metadata"].update(nested_metadata)
        payload["metadata"]["openclaw_channel"] = None
        ...
        trace_metadata = self.last_trace_kwargs["metadata"]
        assert trace_metadata["openclaw_channel"] == "telegram"
        assert trace_metadata["openclaw_sender_username"] == "alice"
        assert trace_metadata["traffic_type"] == "user"
...
class _RecordingLangfuse:
    last_parameters: dict | None = None

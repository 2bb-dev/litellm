...
def test_a_pinned_billing_time_prices_the_totals_and_the_reported_rates_at_one_moment(monkeypatch):
    ...
    with pinned_billing_time(datetime(2026, 1, 1, 2, 30, tzinfo=timezone.utc)):
        explicit_time = datetime(2026, 1, 1, 12, 30, tzinfo=timezone.utc)
        explicit_costs = generic_cost_per_token(
            model="off-peak-model", usage=usage, custom_llm_provider="openai", request_time=explicit_time
        )
        explicit_rates = get_billed_token_rates(
            model="off-peak-model", custom_llm_provider="openai", usage=usage, request_time=explicit_time.timestamp()
        )
    assert explicit_costs == pytest.approx((peak_prompt_cost, peak_completion_cost))
    assert explicit_rates == peak_rates
...
from litellm.litellm_core_utils.llm_cost_calc.utils import parse_prompt_tokens_details, resolve_token_pricing

@pytest.mark.parametrize(
    "writes,creation,expected",
    ((None, None, 0), (0, 0, 0), (300, None, 300), (None, 300, 300), (300, 300, 300), (300, 100, 300), (0, 300, 0)),
)
def test_parse_prompt_tokens_details_cache_write_alias(writes: int | None, creation: int | None, expected: int) -> None:
    ...
    result = parse_prompt_tokens_details(usage)
...
def test_dated_reasoning_rate_precedes_recurring_and_service_tier_rates():
    info = ModelInfo(
        input_cost_per_token=1.0,
        output_cost_per_token=2.0,
        output_cost_per_token_priority=4.0,
        output_cost_per_reasoning_token=6.0,
        output_cost_per_reasoning_token_priority=8.0,
        pricing_periods=[{
            "effective_until": "2026-09-10T00:00:00Z",
            "output_cost_per_reasoning_token": 0.5,
            "off_peak_pricing": {"hours_utc": "00:00-00:00", "output_cost_per_reasoning_token": 0.25},
        }],
    )
    usage = Usage(prompt_tokens=0, completion_tokens=10, completion_tokens_details={"reasoning_tokens": 4})
    costs = generic_cost_per_token(
        "dated-reasoning", usage, "openai", service_tier="priority", model_info=info,
        request_time=datetime(2026, 9, 7, tzinfo=timezone.utc),
    )
    assert sum(costs) == pytest.approx(6 * 4.0 + 4 * 0.5)

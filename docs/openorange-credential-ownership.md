...
## Remaining terminal-proxy interface

`litellm_proxy/` remains unknown because its connection key is not the terminal supplier credential. Platform owns the central proxy and terminal credential custody; Architecture must choose an authenticated correlated terminal-attempt receipt or authoritative scoped terminal-ledger consumption and assign the implementer. The fact must bind actual terminal deployment and immutable registration to the local attempt, including failed fallback attempts. Rewriting the terminal deployment ID to the local proxy deployment ID is not validation. This interface remains required phase-one work in Layer #1188/#1203, not an accepted deferral

...
def load_credentials_from_list(kwargs: dict):
    ...
    stamp: Final = resolve_ownership(kwargs, credential_values, credential_info, credential_ambiguous=ambiguous)
    ...
    if credential_name:
        for key, value in credential_values.items():
            if key not in kwargs:
                kwargs[key] = value
    if credential_name and CredentialAccessor.find_credential(credential_name) is None:
...
def _get_model_info_helper(
    ...
    try:
        ...
        combined_model_name: Final = (
            model if _is_custom_pricing_deployment(model) else potential_model_names["combined_model_name"]
        )
        ...
        if provider_config is not None and not _is_custom_pricing_deployment(model):

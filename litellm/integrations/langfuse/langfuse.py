...
class LangFuseLogger:
    ...
    def _log_langfuse_v2(
        ...
        try:
            ...
            end_user_id: Final = (allowlisted_metadata.get("trace_user_id") or allowlisted_metadata.get("openclaw_user_id") or allowlisted_metadata.get("user_api_key_end_user_id"))
            ...
            if isinstance(_spend_logs_md, dict):
                for _k, _v in _spend_logs_md.items():
                    if isinstance(_k, str) and _k.startswith("openclaw_") and _k not in clean_metadata:
            ...
            # Trace and generation metadata share the standard logging allowlist.
            # Raw steering metadata can contain auth objects and callback credentials.
            emitted_metadata = {
                **log_requester_metadata(redact_user_api_key_info(metadata=allowlisted_metadata)),
                **enrichments,
            }
            spend_logs_metadata = emitted_metadata.get("spend_logs_metadata")
            if isinstance(spend_logs_metadata, dict):
                for key, value in spend_logs_metadata.items():
                    if isinstance(key, str) and key.startswith("openclaw_") and not emitted_metadata.get(key):
                        emitted_metadata[key] = value

            # Mirror allowed metadata onto the trace row so OpenClaw fields reach
            # the Langfuse trace as well as its child generation span.
            #
            ...
            should_write_metadata = existing_trace_id is None or "trace_metadata" in update_trace_keys
            if should_write_metadata:
                if "metadata" in trace_params and isinstance(trace_params["metadata"], dict):
                    trace_params["metadata"] = {**emitted_metadata, **trace_params["metadata"]}
                else:
                    trace_params["metadata"] = emitted_metadata

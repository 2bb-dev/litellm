...
class BaseResponsesAPIStreamingIterator:
    ...
    def __init__(
        ...
        self.terminal_error: Any | None = None
    ...
    @staticmethod
    def _stream_event_index(event: Any, field: str) -> int:
        value = getattr(event, field, 0)
        return value if isinstance(value, int) and value >= 0 else 0

    def _set_stream_content_part(self, item_id: str, content_index: int, part: dict[str, Any]) -> None:
    ...
    def _track_stream_output_event(self, event: Any) -> None:
        ...
        if event_type in (
            ...
            if not item_id:
                return
            self._set_stream_output_index(item_id, getattr(event, "output_index", None))
            content_index = self._stream_event_index(event, "content_index")
        ...
        if event_type == ResponsesAPIStreamEvents.OUTPUT_TEXT_DELTA:
            ...
            self._stream_accumulated_text[key] = self._stream_accumulated_text.get(key, "") + delta
            item = self._ensure_stream_output_item(item_id)
            content = item.setdefault("content", [])
            if not isinstance(content, list):
                content = []
        ...
        if event_type == ResponsesAPIStreamEvents.OUTPUT_TEXT_ANNOTATION_ADDED:
            ...
            self._set_stream_output_index(item_id, getattr(event, "output_index", None))
            content_index = self._stream_event_index(event, "content_index")
            annotation_index = self._stream_event_index(event, "annotation_index")
        ...
        if event_type == ResponsesAPIStreamEvents.REFUSAL_DELTA:
            ...
            delta = str(getattr(event, "delta", "") or "")
            key = (item_id, content_index)
            self._stream_accumulated_refusals[key] = self._stream_accumulated_refusals.get(key, "") + delta
    ...
    def _maybe_backfill_terminal_response_output(self, event: Any) -> None:
        ...
        existing_output = response.get("output") if isinstance(response, dict) else getattr(response, "output", None)
        if existing_output:
            return
        ...
        ordered_ids = sorted(
            ...
            key=lambda item_id: (
                self._stream_output_item_indices.get(item_id, self._stream_output_item_order.index(item_id)),
                self._stream_output_item_order.index(item_id),
            ),
        )
        patched_output = [
            self._stream_output_items[item_id] for item_id in ordered_ids if item_id in self._stream_output_items
        ]
        ...
        try:
            ...
            if _is_json_object(parsed_chunk):
                ...
                if openai_responses_api_chunk and _chunk_type in (
                    ...
                    self._maybe_backfill_terminal_response_output(openai_responses_api_chunk)

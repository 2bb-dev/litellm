...
async def enqueue_spend_logs(
    ...
    *,
    ...
    async with prisma_client._spend_log_transactions_lock:
        ...
        kept, kept_bytes = spend_log_queue_within_budget(queued, sum(spend_log_row_bytes(row) for row in queued), max_bytes)
...
async def dequeue_spend_logs(prisma_client: PrismaClient, limit: int) -> list[dict[str, object]]:
    ...
    async with prisma_client._spend_log_transactions_lock:
        ...
        PrismaClient.spend_log_queue_bytes = max(
            0, sum(spend_log_row_bytes(row) for row in prisma_client.spend_log_transactions)
        )
    return popped
        ...
        try:
            for i in range(n_retry_times + 1):
                try:
                        ...
                        if base_url is not None and db_writer_client is not None:
                            ...
                            if durable_batch:
                                ...
                                for statement_rows in spend_log_write_batches(
                                    batch_with_dates, SPEND_LOG_WRITE_BATCH_MAX_BYTES, SPEND_LOG_WRITE_BATCH_MAX_ROWS
                                ):
                                    isolation_budget = await _create_spend_logs_with_poison_isolation(
                                        SpendLogsRepository(prisma_client), statement_rows, isolation_budget
                                    )
                            verbose_proxy_logger.debug(
                    ...
                    if not _is_transient_spend_log_write_error(e, db_writer_client):
                        raise
                    if n_retry_times > 0:
                        verbose_proxy_logger.warning(
                            ...
                            n_retry_times,
                            len(logs_to_process),
                            type(e).__name__ if encryption_enabled() else str(e),
                        )
                    if i >= n_retry_times:
                        raise
...
async def _total_queued_spend_transactions(prisma_client: PrismaClient) -> int:
    ...
    spend_queue_size: Final = (await spend_log_queue_stats(prisma_client)).count
...
async def update_spend_logs_job(
    ...
    """
    MAX_LOGS_PER_INTERVAL: Final = SPEND_LOG_DB_MAX_LOGS_PER_INTERVAL
    ...
    async with prisma_client._spend_log_write_lock:
        ...
        try:
            ...
            await ProxyUpdateSpend.update_spend_logs(
                ...
                durable_batch=queue_batch.is_durable,
            )
        except asyncio.CancelledError:
            ...
            if _is_transient_spend_log_write_error(spend_log_error, db_writer_client):
                ...
                if len(logs_to_process) > 1:
                    ...
                    await quarantine_spend_log_batch(
                        prisma_client,
                        queue_batch,
                        type(spend_log_error).__name__ if encryption_enabled() else str(spend_log_error),
                    )
            ...
            elif not _is_permanent_spend_log_row_error(spend_log_error):
                await release_spend_log_batch(prisma_client, queue_batch)
            raise
        else:
...
async def drain_spend_logs_queue(
    ...
    if timeout_seconds is not None:
        return await drain_spend_log_queue(
            prisma_client, db_writer_client, proxy_logging_obj, timeout_seconds
        )
...
def _is_transient_spend_log_write_error(
    ...
    if (PrismaDBExceptionHandler.is_database_transport_error(error)
        or PrismaDBExceptionHandler.is_deadlock_error(error)):
        return True
...
async def drain_spend_log_queue(
    ...
    while True:
        remaining = await _total_queued_spend_transactions(prisma_client)
        if remaining == 0:
...
async def _monitor_spend_logs_queue(
    ...
    try:
        ...
        spend_log_error("Spend tracking - initial queue stats failed: %s", str(error), exc=error)

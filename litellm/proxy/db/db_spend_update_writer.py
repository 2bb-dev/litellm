...
class DBSpendUpdateWriter:
    ...
    async def update_database(
        ...
        False for an already charged batch or terminal failure, so the caller
        leaves the key, team, and user counters alone.
        """
    ...
    async def _record_spend_log(
        ...
        if disable_spend_logs is False:
            await self._insert_spend_log_to_db(
                payload=payload, prisma_client=prisma_client, terminal_failure=terminal_failure
            )
        return True

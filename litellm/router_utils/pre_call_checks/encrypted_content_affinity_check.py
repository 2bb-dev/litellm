...
class EncryptedContentAffinityCheck(CustomLogger):
    ...
    async def async_filter_deployments(
        ...
        server-side history referenced by ``previous_response_id`` cannot be rebuilt
        this way, so those continuations remain pinned to the same account or fail.
        The 429/503 split mirrors the originating cooldown's status:
        ...
        if previous_model_id:
            if originating is None:
                raise BadRequestError(
                    message=(
                        "The deployment that produced this previous_response_id is no longer configured "
                        "on this router, and no deployment on the same encryption boundary is available."
                    ),
                    model=model,
                    llm_provider="",
                )
            raise await self._unavailable_origin_error(
                model=model, model_id=model_id, parent_otel_span=parent_otel_span
            )

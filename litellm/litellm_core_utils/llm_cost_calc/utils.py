...
from litellm.utils import _is_custom_pricing_deployment, get_model_info
...
def _apply_off_peak_to_base_costs(
    ...
    base_costs: tuple[float, float, float | None, float, float],
    ...
    rates: Final = apply_off_peak_pricing(
        ...
        TokenRates(
            ...
            cache_creation_rate=cache_creation if cache_creation is not None else prompt,
            reasoning_rate=None,
        ),
    )
    off_peak: Final = _open_off_peak_block(model_info, current_time)
    has_off_peak_write_rate: Final = (
        off_peak is not None and _parse_off_peak_rate(off_peak.get("cache_creation_input_token_cost")) is not None
    )
    return (
        rates.input_rate,
        rates.output_rate,
        rates.input_rate if cache_creation is None and not has_off_peak_write_rate else rates.cache_creation_rate,
        cache_creation_above_1hr
        if off_peak is None
        else _off_peak_rate(off_peak, "cache_creation_input_token_cost_above_1hr", cache_creation_above_1hr),
...
def _get_token_base_cost(
    ...
    *,
    ...
    """
    explicit_threshold: Final = model_info.get("pricing_tier_threshold_inclusive")
    inclusive: Final = explicit_threshold if explicit_threshold is not None else threshold_is_inclusive
    ...
    for key in sorted(threshold_keys, key=_parse_above_token_threshold, reverse=True):
        ...
        if value is not None:
            try:
                ...
                if usage.prompt_tokens > threshold or (inclusive and usage.prompt_tokens == threshold):
...
def _calculate_input_cost(
    ...
    if prompt_tokens_details["image_tokens"]:
        image_token_cost: Final = _get_cost_per_unit(model_info, "input_cost_per_image_token", prompt_base_cost)
...
def _resolve_billed_reasoning_rate(
    ...
    period: Final = _active_token_pricing_period(model_info, _pricing_instant(current_time))
    dated_reasoning: Final = _token_pricing_period_overrides(period).get("output_cost_per_reasoning_token")
    if dated_reasoning is not None:
        return dated_reasoning
    off_peak: Final = _open_off_peak_block(model_info, current_time)
...
def _pricing_instant(request_time: datetime | float | None) -> datetime:
    instant: Final = request_time if request_time is not None else current_billing_time()
    if isinstance(instant, (int, float)):
        return datetime.fromtimestamp(instant, timezone.utc)
    return instant.astimezone(timezone.utc) if instant.tzinfo is not None else instant.replace(tzinfo=timezone.utc)


def _active_token_pricing_period(model_info: ModelInfo, instant: datetime) -> TokenPricingPeriod | None:
...
def _token_pricing_period_overrides(selected: TokenPricingPeriod | None) -> dict[str, float]:
    if selected is None:
        return {}
...
def _prepare_token_pricing(
    model_info: ModelInfo,
    usage: Usage,
    instant: datetime,
    threshold_is_inclusive: bool = False,
) -> ModelInfo:
    selected: Final = _active_token_pricing_period(model_info, instant)
    if selected is None:
        return model_info
    period: Final = _token_pricing_period_overrides(selected)
    inherited_schedule: Final = model_info.get("off_peak_pricing")
    explicit_threshold: Final = model_info.get("pricing_tier_threshold_inclusive")
    inclusive: Final = explicit_threshold if explicit_threshold is not None else threshold_is_inclusive
    dated_fields: Final = frozenset(
        key.rsplit("_above_", 1)[0] if key.endswith("_tokens") else key
        for key in period
        if not key.endswith("_tokens")
        or usage.prompt_tokens > _parse_above_token_threshold(key)
        or (inclusive and usage.prompt_tokens == _parse_above_token_threshold(key))
    )
    schedule: Final = (
        selected["off_peak_pricing"]
        if "off_peak_pricing" in selected
        else {key: value for key, value in inherited_schedule.items() if key not in dated_fields}
        if isinstance(inherited_schedule, Mapping)
        else inherited_schedule
    )
    return cast(ModelInfo, {**model_info, **period, "off_peak_pricing": schedule})
...
def resolve_token_pricing(
    ...
    usage: Usage,
    service_tier: str | None,
    request_time: datetime | float | None,
) -> ModelInfo:
    instant: Final = _pricing_instant(request_time)
    prepared: Final = _prepare_token_pricing(model_info, usage, instant)
    rates: Final = _get_token_base_cost(prepared, usage, service_tier, instant)
    fields: Final = (
        "input_cost_per_token",
    ...
    return cast(ModelInfo, {**prepared, **dict(zip(fields, rates))})
...
def calculate_cache_costs(
    ...
    rates: Final = resolve_token_pricing(model_info, usage, service_tier, request_time)
    details: Final = parse_prompt_tokens_details(usage)
    return (
...
def generic_cost_per_token(
    ...
    billing_time: Final = _pricing_instant(request_time if request_time is not None else current_time)
    model_info = _prepare_token_pricing(
        model_info, usage, billing_time, _uses_inclusive_token_thresholds(custom_llm_provider)
    )
    (
        prompt_base_cost,
        completion_base_cost,
...
def _cost_map_billed_rates(
    ...
    billing_time: Final = _pricing_instant(current_time)
    model_info = _prepare_token_pricing(
        model_info, usage, billing_time, _uses_inclusive_token_thresholds(custom_llm_provider)
    )
    (
    ...
    multiplier: Final = _get_regional_uplift_multiplier(model_info, data_residency) * get_vertex_regional_endpoint_uplift(
        model_info, vertex_location
    )
    geo_multiplier: Final = get_provider_specific_geo_multiplier(model_info=model_info, usage=usage)
    cache_geo_multiplier: Final = 1.0 if _is_custom_pricing_deployment(model) else geo_multiplier
    return BilledTokenRates(
        input_cost_per_token=prompt_base_cost * geo_multiplier,
        output_cost_per_token=completion_base_cost * geo_multiplier,
        cache_read_input_token_cost=cache_read_cost_rate * cache_geo_multiplier,
        cache_creation_input_token_cost=cache_creation_cost_rate * cache_geo_multiplier,
        cache_creation_input_token_cost_above_1hr=cache_creation_cost_above_1hr_rate * cache_geo_multiplier,
        output_cost_per_reasoning_token=reasoning_rate * geo_multiplier,
    ).scaled(multiplier)
...
def get_billed_token_rates(
    ...
    return _cost_map_billed_rates(
        ...
        current_time=_pricing_instant(request_time if request_time is not None else current_time),
        model=model,
    )

...
class GetModelCostMap:
    ...
    _fork_owned_entries: dict | None = None
    ...
    def get_fork_owned_entries(cls) -> dict:
        ...
        if cls._fork_owned_entries is None:
            ...
            cls._fork_owned_entries = {
                model: info for model, info in backup.items() if model.startswith(FORK_OWNED_MODEL_PREFIXES)
            }
...
async def refetch_model_cost_map(
    ...
    _cost_map_source_info.fallback_reason = None
    return _finalize_loaded_model_cost_map(result, include_fork_entries=True)
...
def _finalize_loaded_model_cost_map(
    loaded: ModelCostMapReloaded, *, include_fork_entries: bool = False
) -> ModelCostMapReloaded:
    _cost_map_source_info.source_revision = loaded.revision
    _cost_map_source_info.etag = loaded.etag
    model_cost = (
        merge_fork_owned_entries(loaded.model_cost_map, GetModelCostMap.get_fork_owned_entries())
        if include_fork_entries
        else loaded.model_cost_map
    )
    return replace(loaded, model_cost_map=_finalize_model_cost_map(model_cost))
...
def _retry_remote_fetch_in_background(
    ...
    try:
        ...
        if not GetModelCostMap.validate_model_cost_map(
            ...
            return
        finalized: Final = _finalize_loaded_model_cost_map(result, include_fork_entries=True).model_cost_map
        _cost_map_source_info.source = "remote"
        _cost_map_source_info.fallback_reason = None
        _cost_map_source_info.loaded_at = datetime.now(timezone.utc)
...
def get_model_cost_map(
    ...
    _cost_map_source_info.source = "remote"
    _cost_map_source_info.fallback_reason = None
    return _finalize_loaded_model_cost_map(outcome, include_fork_entries=True).model_cost_map

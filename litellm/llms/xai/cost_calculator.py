...
from litellm.utils import _is_custom_pricing_deployment
...
def cost_per_token(model: str, usage: Usage, request_time: datetime | float | None = None) -> tuple[float, float]:
    ...
    if reported_cost is not None and not _is_custom_pricing_deployment(model):
    ...
    prompt_cost, completion_cost = generic_cost_per_token(
        model=model, usage=modified_usage, custom_llm_provider="xai", request_time=request_time
    )
